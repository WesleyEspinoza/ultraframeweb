...
const FAQS = [
  ...
  { q: "Is this safe — will it trip antivirus software?", a: "UltraFrame is safe to use, but we want to be upfront: we have not paid Microsoft for extended SmartScreen reputation signing, so Windows Defender and many other antivirus programs may flag the installer or app — especially on first run. That is a common false positive for software that changes registry and system settings (the same kinds of changes Windows' own tools make). UltraFrame does not contain malware. If you see a warning, allow or whitelist UltraFrame in your antivirus, or use the option your AV provides for a known-safe app. We also document what the app changes so you can see exactly what it does." },
  { q: "How is this different from free tools like MSConfig or game boosters?", a: "Those tools scratch the surface. UltraFrame applies 50+ targeted system changes across the CPU scheduler, network stack, power management, VRAM, startup services, GPU settings, and more — sequenced in the correct order with safety checks and one-click rollback. It's hours of expert manual work packaged into a guided workflow." },
  { q: "Can I use this on more than one PC?", a: "Yes. One UltraFrame key can be used on up to 3 computers." },
  { q: "How long do updates last?", a: "Your purchase includes updates for as long as UltraFrame is actively supported. During public beta, we ship improvements regularly — bug fixes, refinements, and features on the roadmap." },
  { q: "What does public beta mean?", a: "UltraFrame is fully usable today, but it is still in beta. That means you get early-access pricing, and we are actively building — planned updates will add features, polish the experience, and expand what the optimizer can do. You may see UI or behavior change between versions as we improve the product. Your license includes those updates while development continues." },
  ...
  { q: "What PC specs do I need for the AI assistant (Qwen3 8B)?", a: "For the local Qwen3 8B assistant, plan for at least 12 GB system RAM, an NVIDIA GeForce GTX 1060 with 6 GB VRAM (or better), ~10 GB free disk space for the model, and Windows 10 (1903+) or Windows 11. More VRAM (8 GB+) and 16 GB RAM give noticeably faster, smoother chat. The core UltraFrame optimizer alone is much lighter — see the Minimum specs section on the homepage for full details." },
];
