  ...
  if (!config) {
    const hint =
      process.env.VERCEL === "1" || process.env.NETLIFY === "true"
        ? " Add EMAIL_* variables in your hosting dashboard (not only local .env)."
        : " Add EMAIL_HOST, EMAIL_USER, EMAIL_PASS, and TO_EMAIL to .env and restart the dev server.";
    return {
      status: "error",
      message: `License email is not configured on the server.${hint}`,
    };
  }
